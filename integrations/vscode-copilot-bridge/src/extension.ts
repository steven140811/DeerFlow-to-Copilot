...
let lastRequestProbe: {
  status: 'unknown' | 'pending' | 'ready' | 'error';
...
function getConfiguredModel(): string {
  return getConfig().get<string>('model', 'gpt-5.4').trim();
}
...
function getConfiguredReasoningEffort(): string | undefined {
  return normalizeReasoningEffort(getConfig().get<string>('reasoningEffort', 'xhigh'));
}

function getBridgeAddress(): string {
  const host = getConfig().get<string>('host', '127.0.0.1');
  const port = getConfig().get<number>('port', 8765);
  return `http://${host}:${port}`;
}

function getRequestTimeoutMs(): number {
  const configured = getConfig().get<number>('requestTimeoutMs', 90000);
  return Number.isFinite(configured) && configured > 0 ? configured : 90000;
}

function getLaunchTimeoutMs(): number {
  const configured = getConfig().get<number>('launchTimeoutMs', DEFAULT_LAUNCH_TIMEOUT_MS);
...
function getSavedProfilePath(): string {
  const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
  return path.join(appData, 'DeerFlow', 'vscode-to-deerflow.profile.json');
}

function getDeerFlowUiUrl(): string {
  try {
    const raw = JSON.parse(fs.readFileSync(getSavedProfilePath(), 'utf8')) as JsonObject;
...
function getLaunchTerminal(context: vscode.ExtensionContext): vscode.Terminal {
  if (launchTerminal) {
...
function getTerminalCommand(command: string): string {
  return process.platform === 'win32' ? command : command;
}

async function getUrlStatus(target: string, timeoutMs = 3000): Promise<number | undefined> {
  let parsed: URL;
  try {
...
async function waitForUiReady(baseUrl: string, timeoutMs: number): Promise<boolean> {
...
async function openDeerFlowUi(): Promise<void> {
  await vscode.env.openExternal(vscode.Uri.parse(getDeerFlowUiUrl()));
}

async function configureDeerFlowAccount(context: vscode.ExtensionContext): Promise<void> {
...
async function ensureLocalDeerFlow(context: vscode.ExtensionContext): Promise<boolean> {
  ...
  if (!ready) {
    void vscode.window.showWarningMessage(
      'DeerFlow local services are still starting. The UI will be opened anyway; check the "DeerFlow Local Services" terminal if it is not ready yet.'
    );
  }
  return ready;
}

async function launchWorkbench(context: vscode.ExtensionContext): Promise<void> {
...
function setRequestProbe(status: 'unknown' | 'pending' | 'ready' | 'error', modelId?: string, error?: string): void {
  lastRequestProbe = {
    status,
...
function reportBridgeState(message: string): void {
...
function readJsonBody(request: http.IncomingMessage, maxBytes = 4 * 1024 * 1024): Promise<JsonObject> {
...
function isAuthorized(request: http.IncomingMessage): boolean {
  const token = getConfig().get<string>('token', '');
  if (!token) {
    return true;
  }
    ...
    return content
      .map(part => {
        ...
        if (part && typeof part === 'object') {
          const typedPart = part as JsonObject;
...
function toolCallsFromOpenAiMessage(message: JsonObject): vscode.LanguageModelToolCallPart[] {
  ...
  for (const rawToolCall of rawToolCalls) {
    ...
    const fn = typeof toolCall.function === 'object' && toolCall.function ? (toolCall.function as JsonObject) : {};
...
function openAiMessagesToVsCode(messages: unknown): vscode.LanguageModelChatMessage[] {
...
function openAiToolsToVsCode(tools: unknown): vscode.LanguageModelChatTool[] | undefined {
  ...
  for (const rawTool of tools) {
    ...
    const fn = typeof tool.function === 'object' && tool.function ? (tool.function as JsonObject) : tool;
...
async function listCopilotModels(selector: vscode.LanguageModelChatSelector = { vendor: 'copilot' }) {
  return vscode.lm.selectChatModels(selector);
}

async function selectCopilotModel(requestedModel: unknown): Promise<vscode.LanguageModelChat> {
  const requested = typeof requestedModel === 'string' ? requestedModel : '';
  const configuredModel = getConfiguredModel();
  const configuredFamily = getConfig().get<string>('family', '');
...
async function handleModels(response: http.ServerResponse): Promise<void> {
...
function requestOptionsFromBody(body: JsonObject): vscode.LanguageModelChatRequestOptions {
...
async function probeCopilotRequest(model: vscode.LanguageModelChat): Promise<void> {
...
async function handleChatCompletion(body: JsonObject, response: http.ServerResponse): Promise<void> {
...
async function startBridge(warmupModel: boolean): Promise<void> {
  if (server) {
  ...
  reportBridgeState(
    `DeerFlow Copilot bridge listening on ${getBridgeAddress()}${cachedModel ? ` via ${cachedModel.id}` : ''}.`
  );
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    outputChannel,
    statusItem,
    vscode.commands.registerCommand('deerflowCopilotBridge.start', async () => {
      try {
        await startBridge(true);
      } catch (error) {
