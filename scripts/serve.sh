...
SOURCE_NGINX_CONF="$REPO_ROOT/docker/nginx/nginx.local.conf"
GENERATED_NGINX_CONF="$REPO_ROOT/temp/nginx.local.generated.conf"
ACTIVE_NGINX_CONF="$SOURCE_NGINX_CONF"
FRONTEND_HOST="${DEERFLOW_FRONTEND_HOST:-127.0.0.1}"
REQUESTED_FRONTEND_PORT="${DEERFLOW_FRONTEND_PORT:-3000}"
...
PORT_PROBE_PYTHON="$(pick_python_bin 2>/dev/null || true)"

_is_port_bindable() {
    local host=$1
    local port=$2

    if [ -z "$PORT_PROBE_PYTHON" ]; then
        ! _is_port_listening "$port"
        return
    fi

    "$PORT_PROBE_PYTHON" - "$host" "$port" >/dev/null 2>&1 <<'PY'
import socket
import sys
...
_http_ok() {
    ...
    "$PORT_PROBE_PYTHON" - "$url" >/dev/null 2>&1 <<'PY'
import sys
...
resolve_frontend_port() {
    ...
    local upper_bound=$((requested + 10))

    while [ "$candidate" -le "$upper_bound" ]; do
        if _is_port_bindable "$FRONTEND_HOST" "$candidate"; then
            echo "$candidate"
            return 0
        fi
    ...
    echo "✗ Unable to find a free frontend port in the range ${requested}-${upper_bound}." >&2
    return 1
}
...
generate_nginx_conf() {
    ...
    sed \
        -e "s/server 127\.0\.0\.1:3000;/server ${FRONTEND_HOST}:${FRONTEND_PORT};/" \
        "$SOURCE_NGINX_CONF" > "$GENERATED_NGINX_CONF"

    ACTIVE_NGINX_CONF="$GENERATED_NGINX_CONF"
}

build_nginx_cmd() {
    case "$(uname -s)" in
        MINGW*|MSYS*|CYGWIN*)
            NGINX_CMD="nginx -c '$ACTIVE_NGINX_CONF' -p '$REPO_ROOT' > logs/nginx.log 2>&1"
            ;;
        *)
            NGINX_CMD="nginx -g 'daemon off;' -c '$ACTIVE_NGINX_CONF' -p '$REPO_ROOT' > logs/nginx.log 2>&1"
            ;;
    esac
}
...
stop_all() {
    ...
    local nginx_conf="$SOURCE_NGINX_CONF"

    if [ -f "$GENERATED_NGINX_CONF" ]; then
        nginx_conf="$GENERATED_NGINX_CONF"
    fi
...
# Frontend command
if $DEV_MODE; then
    FRONTEND_CMD="pnpm exec next dev --turbo --hostname $FRONTEND_HOST --port $FRONTEND_PORT"
else
    if ! PYTHON_BIN="$(pick_python_bin)"; then
        echo "Python is required to generate BETTER_AUTH_SECRET."
        exit 1
    fi
    BETTER_AUTH_SECRET="$($PYTHON_BIN -c 'import secrets; print(secrets.token_hex(16))')"
    FRONTEND_CMD="env BETTER_AUTH_SECRET=$BETTER_AUTH_SECRET pnpm exec next build && env BETTER_AUTH_SECRET=$BETTER_AUTH_SECRET pnpm exec next start --hostname $FRONTEND_HOST --port $FRONTEND_PORT"
fi
...
# Pick a Python for the extras detector. On Windows/Git Bash, prefer the
# native `python` launcher because `python3` can resolve to an MSYS shim that
# exits non-zero for otherwise healthy local runs.
DETECT_PYTHON="$(pick_python_bin 2>/dev/null || true)"
...
FRONTEND_PORT="$(resolve_frontend_port "$REQUESTED_FRONTEND_PORT")" || exit 1
if [ "$FRONTEND_PORT" != "$REQUESTED_FRONTEND_PORT" ]; then
    echo "ℹ Frontend port $REQUESTED_FRONTEND_PORT is already in use; using localhost:$FRONTEND_PORT for this DeerFlow session."
fi
...
run_gateway_service() {
    ...
    local elapsed=0
    local interval=1
    local logfile="logs/gateway.log"
    local health_url="http://127.0.0.1:8001/health"

    if _is_port_listening 8001; then
        echo "✗ Gateway cannot start because port 8001 is already in use."
        echo "  If it belongs to this worktree, run 'make stop'; otherwise free the port manually."
        cleanup 1
    fi
    ...
    while ! _http_ok "$health_url"; do
        if [ "$elapsed" -ge "$timeout" ]; then
            echo ""
            echo "✗ Gateway failed to start on localhost:8001 after ${timeout}s"
            echo "✗ Gateway failed to start."
            [ -f "$logfile" ] && tail -20 "$logfile"
            cleanup 1
        fi

        printf "\r  Waiting for Gateway on port 8001... %ds" "$elapsed"
        sleep "$interval"
        elapsed=$((elapsed + interval))
    done

    printf "\r  %-60s\r" ""
    echo "✓ Gateway started on localhost:8001"
}
